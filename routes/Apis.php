...
use MUONLINECORE\App\Widen\Routes\Apis\Apis as Route;
use MUONLINECORE\App\Http\Controllers\ApisController;
use MUONLINECORE\App\Http\Middleware\ApiTokenAuthentication;
use MUONLINECORE\App\Http\Middleware\AuthenticationMiddleware;
use MUONLINECORE\App\Http\Middleware\SanctumMiddleware;
	...
	public function apis() {
		Route::namespace('wpsp')->version('v1')->group(function() {
			Route::name('api-token.')->prefix('api-token')->group(function() {
				...
				])->post('test', [ApisController::class, 'testApiToken'])->name('test');
			});
