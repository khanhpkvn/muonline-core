...
use MUONLINECORE\App\Http\Requests\UsersUpdateRequest;
...
//			throw new \MUONLINECORE\App\Exceptions\QueryException($wpdb->last_query, $data, 'Failed to update post');
			}
		}
		catch (\Throwable $e) {
	...
	public function matchedCurrentAccess() {
		...
		if ($isListPage) {
			add_action('current_screen', function($screen) {
				$this->table = new \MUONLINECORE\App\WordPress\ListTables\Users();
...
//				wp_die('You do not have permission to edit this user!');
		...
		if ($action == 'edit' && $id) {
			...
			muoc_view_inject('admin-pages.wpsp.users', function($view) use ($selectedUser) {
