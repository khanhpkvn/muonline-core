...
@section('content')
    ...
    @if (!isset($current_user) || !$current_user)
        <form method="POST" style="border: 1px solid red; padding: 20px;" action="{{ muoc_route('Apis', 'auth.login', true) }}">
            @csrf
            <input type="hidden" name="action" value="login"/>
            <h3 style="margin-top: 0;">CUSTOM LOGIN FORM</h3>
            <p>This is custom login form using: <b>wpsp-auth</b></p>
				<?php muoc_nonce_field('wp_rest'); ?>
    ...
    @else
        <form method="POST" style="border: 1px solid red; padding: 20px;" action="{{ muoc_route('Apis', 'auth.logout', true) }}">
            @csrf
            ...
            @php
                try {
					...
					echo '<small style="color: #cc0000;font-family: monospace;">$current_user->toArray()</small><br/>';
					...
					echo '<small style="color: #cc0000;font-family: monospace;">$current_user->permissions->pluck(\'name\')->toArray()</small><br/>';
...
//        			    $permissions = $current_user->roles()->with('permissions')->get()->pluck('permissions')->flatten()->unique('id')->pluck('name')->toArray();
                        $permissions = [];
						try {
                            $rolesWithPermissions = $current_user ? $current_user->roles()->with('permissions')->get() : [];
					...
					echo '<small style="color: #cc0000;font-family: monospace;">$rolesWithPermissions = $current_user->roles()->with(\'permissions\')->get();</small><br/>';
